...
static CK_RV deserialize_indirect_attribute(void *obj,
					    CK_ATTRIBUTE_PTR attribute)
{
	CK_ULONG count;
	CK_ATTRIBUTE_PTR attr;
	 ...
	 */
	rv = deserialize_ck_attributes(((char *)obj) +
					sizeof(struct sks_object_head),
			attr, count);
	return rv;
}
...
CK_RV deserialize_ck_attributes(char *in, CK_ATTRIBUTE_PTR attributes,
				CK_ULONG count)
{
	CK_ATTRIBUTE_PTR cur_attr = attributes;
	CK_ULONG n;
